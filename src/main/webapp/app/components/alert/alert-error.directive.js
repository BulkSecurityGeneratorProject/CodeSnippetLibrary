(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('codeSnippetLibraryApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-codeSnippetLibraryApp-error');
                var entityKey = httpResponse.headers('X-codeSnippetLibraryApp-params');
                if (errorHeader) {
